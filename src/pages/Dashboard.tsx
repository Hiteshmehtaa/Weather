...
export const Dashboard: React.FC = () => {
  ...
  const { data: weather, isLoading: weatherLoading } = useWeatherData(location);
  const { data: aqData, isLoading: aqLoading } = useAirQualityData(location);
  ...
  const visibilityKm = hourly.visibility[0] / 1000;
    ...
    <>
        ...
        <div className="lg:col-span-2 glass-card rounded-xl p-8 flex flex-col justify-between min-h-[400px] relative overflow-hidden group">
          ...
          <div className="relative z-10 flex justify-between items-start">
            <div>
              ...
              <p className="text-on-surface-variant text-lg capitalize">{getWeatherDescription(current.weather_code)}</p>
            </div>
            <div className="flex gap-2">
              <span className="px-4 py-1.5 bg-primary/10 text-primary border border-primary/20 rounded-full text-xs font-bold">LIVE UPDATE</span>
            </div>
          </div>
          ...
          <div className="relative z-10 flex items-center gap-12 mt-4">
            <div className="flex items-baseline">
              <span className="text-[8rem] font-bold tracking-tighter leading-none text-glow">
                {formatTemperature(current.temperature_2m, tempUnit).replace('°C', '').replace('°F', '')}
              </span>
            ...
            <div className="flex flex-col gap-2">
              <span className="material-symbols-outlined text-8xl text-secondary" style={{ fontVariationSettings: "'FILL' 1" }}>
                {current.weather_code > 50 ? 'rainy' : 'cloud_queue'}
              </span>
              <div className="text-on-surface-variant flex items-center gap-2">
                <span className="material-symbols-outlined text-sm">thermostat</span>
                <span className="text-sm font-medium">Feels like {Math.round(current.apparent_temperature)}°</span>
              </div>
            </div>
          </div>
          ...
          <div className="relative z-10 grid grid-cols-4 pt-8 mt-8 border-t border-outline-variant/10 gap-4">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-on-surface font-semibold text-sm xl:text-base">{current.relative_humidity_2m}%</span>
            </div>
            <div className="flex flex-col">
              <span className="font-label uppercase tracking-[0.05em] text-[0.6875rem] text-on-surface-variant mb-1">Precipitation</span>
              <span className="text-on-surface font-semibold text-sm xl:text-base">{current.precipitation} mm</span>
            </div>
          </div>
        </div>
      ...
      <section className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <div className="bg-surface-container rounded-lg p-6 hover:bg-surface-container-high transition-colors group">
          ...
          <div className="flex items-baseline gap-1">
            <span className="text-2xl font-bold text-on-surface">{current.relative_humidity_2m}</span>
        ...
        <div className="bg-surface-container rounded-lg p-6 hover:bg-surface-container-high transition-colors group">
          ...
          <div className="flex items-baseline gap-1">
            <span className="text-2xl font-bold text-on-surface">{Math.max(...hourly.wind_speed_10m.slice(0, 24))}</span>
        ...
        <div className="bg-surface-container rounded-lg p-6 hover:bg-surface-container-high transition-colors group">
          ...
          <div className="flex items-baseline gap-1">
            <span className="text-2xl font-bold text-on-surface">{daily.precipitation_probability_max[0]}</span>
            <span className="text-sm text-on-surface-variant">%</span>
          </div>
        </div>
        ...
        <div className="bg-surface-container rounded-lg p-6 hover:bg-surface-container-high transition-colors group">
          ...
          <div className="flex items-baseline gap-1 mt-1">
            <span className="text-xl font-bold text-secondary">{Math.round(daily.temperature_2m_min[0])}°</span>
            <span className="text-sm text-on-surface-variant mx-1">/</span>
            <span className="text-xl font-bold text-error">{Math.round(daily.temperature_2m_max[0])}°</span>
          </div>
        </div>
      </section>
